...
public class TomatoClientVer2 extends JFrame implements ActionListener {
	...
	JButton jbtn_imoticon	  = new JButton("이모티콘");
	...
	//JTeatArea에는 문자열을 그리는 개념이 아니라 출력하는 개염이라서 부분색상 변경, 문자열 그리기등이 안됨.
	...
	public void init() {
		try {
			...
			oos.writeObject(130+"#"+nickName);//말했잖아
		...
		if(jbtn_one == obj) {
			...
			try {
				oos.writeObject(Protocol.CHANGE
						...
						+Protocol.seperator+nickName+"님의 대화명이 "+afterName+"으로 변경."+msg);
			} catch (Exception e) {
